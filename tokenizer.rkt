...
(define (tokenize expr)
  (let ((parts (string-split expr)))
    (cond
      ...
      [(function? functions (first parts)) (%tokenize-function parts)]
...
(define (%tokenize-function parts)
  (cond ((null? parts) '())
        ((function? functions (first parts)) (cons (first parts) (%tokenize-function (cdr parts))))
        ((char-numeric? (car(string->list(car parts))))
         (cons (string->number(list->string (list (car(string->list(car parts))))))
               (%tokenize-function (cdr parts))))))
